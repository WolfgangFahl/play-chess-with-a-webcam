...
class Board(object):
    """This class is used to hold the state of a chessboard with pieces positions and the current player's color which player needs to play)."""
    ...
    def _move(self, fromCell, toCell):
        ...
        if len(fromCell) != 1 or len(toCell) != 1 or self._emptyCell(fromCell[0]) or (self.board[fromCell[0]][COLOR] != self.toPlay) or self._sameColor(fromCell[0], toCell[0]):
            raise RejectedMove(
                "During move: Not a regular move, or fromCell is empty, or moving with opponent's piece, or eating it's own piece")
        ...
        if self._tryToPromote(toCell[0]) == RESULT['SUCCESS']:
            lastMove = lastMove + "q"
    ...
    def _eat(self, fromCell, toCell):
        ...
        if len(fromCell) != 2 or len(toCell) != 1 or self._sameColor(fromCell[0], fromCell[1]):
            raise RejectedMove(
                "During eat: fromCell or toCell size not fit, or eating it's own piece")
        ...
        # "En passant": The eaten piece is not where the eating piece will go
        self.board[fromCell[(i + 1) % 2]] = None
    ...
    def _castle(self, fromCell, toCell):
        ...
        if len(fromCell) != 2 or len(toCell) != 2:
            raise RejectedMove(
                "During Castle: fromCell or toCell size not fit")
        ...
        if not self._emptyCell(toCell[0]) or not self._emptyCell(toCell[1]) or not self._sameColor(fromCell[0], fromCell[1]) or (self.board[fromCell[0]][COLOR] != self.toPlay):
            raise RejectedMove(
                "During Castle: destination cells not available, or not trying to casle with own team")
        ...
        if (not (self.board[fromCell[0]][PIECE] == P['r'] or self.board[fromCell[0]][PIECE] == P['k']) or
            ...
            raise RejectedMove(
                "During Castle: trying to castle without king or without rook")
    ...
    def _validateMove(self, fromCell, toCell):
        ...
        if not ISVALID[Pi[piece]](fromCell, toCell):
            raise RejectedMove(
                "Move violation : The current piece cannot move that way!")
    ...
    def _partMoves(self, move):
        ...
        if len(stepIn) == 1:  # It is the ending position, for sure
            ...
            if self.board[stepOut[0]][PIECE] == P['p']:
                # position where opponent pawn may be
                pos = stepIn[0][0] + stepOut[0][1]
        ...
        if isInList(nextCell0, opponent):  # 1: eat the opponent on the played's dominated cell
            ...
            # 2: player-space-opponent : AMBIGUOUS, we assume player dont eat opponent's piece and that it steps in empty space
            if isInList(nextCell1, opponent):
                stepIn.append(nextCell0)
            elif isInList(nextCell1, empty):
                nextCell2 = GetDominated(nextCell1, self.dominator)
                # 3: player-space-space-opponent : player do not eat opponent's piece. it steps on last empty square
                if isInList(nextCell2, opponent):
                    stepIn.append(nextCell1)
                # 4 : player-space-space-space : player stand in the space of the middle
                elif isInList(nextCell2, empty):
                    stepIn.append(nextCell1)
    ...
    def position(self):
        it = iter(sorted(self.board.items()))
        posstr=""
        for i in range(0,64):
            data = next(it)
            str = data[0]
